...
const HomePage = {
  ...
  <div>
    <div v-if="photos.length===0 && !loading" class="empty-state">
      ...
      <p>等待第一束光</p>
      <button v-if="user" class="btn btn-primary" @click="$router.push('/upload')" style="margin-top:1rem">上传</button>
      <button v-else class="btn" @click="$router.push('/register')" style="margin-top:1rem">加入</button>
    </div>

    <!-- 流动墙 -->
    <div v-if="photos.length>0" class="flow-wall" ref="wall"
      @wheel.prevent="onWheel">
      <div class="flow-row slow">
        <div v-for="p in photos" :key="'s-'+p.id" class="flow-card"
          @click="$router.push('/photo/'+p.id)">
          <img v-if="p.media_type==='photo'" :src="'/uploads/'+p.filename" loading="lazy"
            @load="e=>e.target.classList.add('loaded')"
            @error="e=>{e.target.style.display='none';e.target.nextElementSibling?.style?.display!=='none'||(e.target.style.minHeight='200px')}">
          <video v-else :src="'/uploads/'+p.filename" muted loop
    ...
    <div v-if="hasMore && photos.length>0" style="text-align:center;padding:1.5rem">
